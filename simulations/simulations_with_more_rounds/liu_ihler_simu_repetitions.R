...
for(sim_i in 1:SM){
  ...
  for(i in 1:N){
    ...
    lambda.max = max(max(S_temp - diag(p)), 
                     -min(S_temp - diag(p)))
    lambda.min = lambda.min.ratio*lambda.max
    lambdas = exp(seq(log(lambda.max), 
                      log(lambda.min), 
    ...
    for(lambda in lambdas){
      tvgl_res$icov[[j]] = glassoFast::glassoFast(S = S_temp,
                                                  rho = lambda)$wi
  ...
  for(i in 1:N){
    ...
    ebic = ifelse(ebic %in% c(-Inf, NA, Inf), Inf, ebic)
...
fn = paste0("simulations/simulations_with_more_rounds/results/liu_ihler/liu_ihler_mod_", n, "_", p, "_", N, ".txt")
