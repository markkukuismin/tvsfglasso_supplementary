...
stars_glasso_sf <- function(X = NULL, alpha = NULL, K = 2, stars_thr = 0.1, subsamp_ratio = NULL, rep_num = 20, verbose = TRUE){
  ...
  for(i in 1:rep_num){
    ...
    for(j in 1:nalpha){
      A <- glasso_sf(X=X[ind, ], alpha=alpha[j], K=K)$Theta_hat
      A <- abs(A)
      A[A < 1e-04] <- 0
      A[A != 0] <- 1
